...
2. Set the view controller scene's class to `PhotoDetailViewController`.
3. Add a `UIImageView`, a `UIButton`, and a `UITextField`. Change the button's title to "Add Photo"
4. Add a `UINavigationItem` then a `UIBarButtonItem` to the right of the navigation bar. Set its "System Item" to "Save".
5. Create outlets from the image view and text field.
6. Create an action from the "Add PHoto" button called `addPhoto`, and an action from the bar button item called `savePhoto`.
7. Create a "Show" segue from the "Add" bar button item in the collection view scene to this view controller. Give it an identifier.
8. Create another "Show" segue to this view controller from the collection view cell. Give it an identifier.
9. Create the following variables in this subclass:
...
2. Set the view controller scene's class to `ThemeSelectionViewController`.
3. Add a label that tells the user to select a theme that they would like to use.
4. Add two buttons. One should day "Dark" for the dark theme, and the other should say the other theme that you chose to make in the `ThemeHelper` such as "Blue".
5. Add an action from each button. Call them `selectDarkTheme` and `selectYourColorHereTheme`.
6. Create a "Present Modally" segue from the "Select Theme" bar button item in the collection view scene to this view controller. Give it an identifier.
7. Create a `themeHelper: ThemeHelper?` variable in this subclass.
